...
class ProductPaymentController extends GetxController implements GetxService {
  ...
  double _depositePrice = 0.0;
  double get depositePrice => _depositePrice;


  double _bookingFee = 2.99;
  double get bookingFee => _bookingFee;

  double _processingFee = 0.0;
  double get processingFee => _processingFee;

  double _subTotal = 0.0;
  double get subTotal => _subTotal;

  double _payAtSalon = 0.0;
  double get payAtSalon => _payAtSalon;

  double _payNow = 0.0;
  double get payNow => _payNow;
  ...
  Future<void> getMyWalletAmount() async {
    ...
    print("HelloMyWalletAmount=====>"+response.bodyString.toString());
    apiCalled = true;
  ...
  void calculateAllCharge() {
    double totalPrice =
    double.parse((  Get.find<ProductCartController>().totalPrice +
        Get.find<ProductCartController>().orderTax +
        deliveryPrice - discount).toStringAsFixed(2));
      ;
    ...
    if (_selectedCoupon.discount != null && _selectedCoupon.discount != 0) {
      double percentage(numFirst, per) {
        ...
        var calculatedDiscount =  percentage(Get.find<ProductCartController>().totalPrice,
            _selectedCoupon.discount);
      _discount =double.parse((calculatedDiscount).toStringAsFixed(2));

      // totalPrice = Get.find<ProductCartController>().totalPrice +
      //     deliveryPrice -
      //     discount;
    }
  ...
  //Commented By geeks on 2 Aug 2023
/*  void oldCalculateAllCharge() {

    _depositePrice = double.parse((Get.find<ProductCartController>().totalPrice * 20 / 100).toStringAsFixed(2));
    _processingFee = double.parse((depositePrice * 3.5 / 100).toStringAsFixed(2));

    // double totalPrice = Get.find<ProductCartController>().totalPrice +
    //     Get.find<ProductCartController>().orderTax +
    //     deliveryPrice;
    //
    double totalPrice =  double.parse((depositePrice + bookingFee + processingFee).toStringAsFixed(2));
    _subTotal = Get.find<ProductCartController>().totalPrice + deliveryPrice - discount;
    ...
    if (_selectedCoupon.discount != null && _selectedCoupon.discount != 0) {
      ...
      _depositePrice = double.parse((subTotal * 20 / 100).toStringAsFixed(2));
      _processingFee = double.parse((depositePrice * 3.5 / 100).toStringAsFixed(2));
    }

    _payAtSalon =  double.parse((subTotal - depositePrice).toStringAsFixed(2));
    _payNow = double.parse((depositePrice + bookingFee + processingFee).toStringAsFixed(2));

    // _payNow = depositePrice + bookingFee + processingFee;
    walletDiscount = balance;
    totalPrice = payNow;

//Commented by geeks because discount field  reflect total amount
  *//*  if (isWalletChecked == true) {
      if (totalPrice <= walletDiscount) {
        walletDiscount = totalPrice;
        totalPrice = totalPrice - walletDiscount;
  ...
  void onCheckout() {
    print("helloHere===>"+paymentId.toString());
    if (paymentId == 1) {
      createOrder();
      // cod
      //  Order API call
