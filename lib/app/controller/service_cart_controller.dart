...
class ServiceCartController extends GetxController implements GetxService {
  ...
  bool checkServiceInCart(int id) {
    ...
    return savedInCart.services!.where((element) => element.id == id).isNotEmpty;
  }

  bool checkPackageInCart(int id) {
    return savedInCart.packages!.where((element) => element.id == id).isNotEmpty;
  }
  ...
  void calcuate() {
    ...
    _totalItemsInCart = _savedInCart.services!.length + _savedInCart.packages!.length;
    _totalPrice = total;
    calculateAllCharge();
    update();
  }
  ...
  calculateAllCharge() {
    print("HelloPriceCollectionisOnWork");
    _totalPrice = double.parse((_totalPrice).toStringAsFixed(2));
    // double totalPrice = _totalPrice + orderTax;
// Commented By Geeks on Aug 4 2023
    double totalPrice = _totalPrice;

    _depositePrice = (double.parse((_totalPrice).toStringAsFixed(2)) * 20) / 100;
    print("HelloPriceCollectionisOnWork" + depositePrice.toString());

    _processingFee = (double.parse((depositePrice).toStringAsFixed(2)) * 3.5) / 100;

    _payNow = double.parse((depositePrice + bookingFee + processingFee).toStringAsFixed(2));

    _payAtSalon = double.parse((totalPrice - depositePrice).toStringAsFixed(2));
