...
class IndividualPaymentController extends GetxController implements GetxService {
  ...
  Future<void> getSalonDetails() async {
    var response = await parser.individualDetails({"id": Get.find<IndividualSlotController>().uid});
    apiCalled = true;
    if (response.statusCode == 200) {
      Map<String, dynamic> myMap = Map<String, dynamic>.from(response.body);
      var body = myMap['data'];
  ...
  void calculateDistance() {
    ...
    if (addressInfo.lat != null && addressInfo.lng != null && _individualInfo.lat != null && _individualInfo.lng != null) {
  ...
  void calculateAllCharge() {
    double totalPrice = Get.find<ServiceCartController>().totalPrice + Get.find<ServiceCartController>().orderTax + deliveryPrice;
  ...
  void onCoupon(String offerId, String offerName) {
    ...
    Get.toNamed(AppRouter.getCouponRoutes(), arguments: ['individual-service', offerId, offerName]);
  }
  ...
  void onSelectAddress() {
    ...
    Get.toNamed(AppRouter.getAddressList(), arguments: ['individual', selectedAddressId]);
  }
  ...
  void onCheckout() {
    if (paymentId == 1) {
      ...
      // stripe payment
      Get.toNamed(AppRouter.getStripePay(), arguments: [grandTotal, gateway!.currencyCode.toString(), 'individual']);
    } else if (paymentId == 3) {
      Get.delete<WebPaymentController>(force: true);
      var paymentURL = AppConstants.payPalPayLink + grandTotal.toString();
      Get.toNamed(AppRouter.getWebPayment(), arguments: [payMethodName, paymentURL, 'individual']);
      // paypal
    } else if (paymentId == 4) {
      // paytm
      Get.delete<WebPaymentController>(force: true);
      ...
      // razorpay
      Get.delete<WebPaymentController>(force: true);
      var paymentPayLoad = {'amount': double.parse((grandTotal * 100).toStringAsFixed(2)).toString(), 'email': parser.getEmail(), 'logo': '${parser.apiService.appBaseUrl}storage/images/${parser.getAppLogo()}', 'name': parser.getName(), 'app_color': '#f47878'};
      ...
      Get.delete<WebPaymentController>(force: true);
      var gateway = paymentList.firstWhereOrNull((element) => element.id.toString() == paymentId.toString());
      var paymentPayLoad = {'amount': grandTotal.toString(), 'email': parser.getEmail(), 'phone': parser.getPhone(), 'name': parser.getName(), 'code': gateway!.currencyCode.toString(), 'logo': '${parser.apiService.appBaseUrl}storage/images/${parser.getAppLogo()}', 'app_name': Environments.appName};
  ...
  Future<void> createOrder() async {
    ...
    var param = {"uid": parser.getUID(), "freelancer_id": Get.find<ServiceCartController>().salonId, "salon_id": 0, "specialist_id": 0, "appointments_to": 1, "address": jsonEncode(addressInfo), "items": jsonEncode(Get.find<ServiceCartController>().savedInCart), "coupon_id": selectedCoupon.code != null ? selectedCoupon.id : 0, "coupon": selectedCoupon.code != null ? jsonEncode(selectedCoupon) : 'NA', "discount": discount, "distance_cost": deliveryPrice, "total": Get.find<ServiceCartController>().totalPrice, "serviceTax": Get.find<ServiceCartController>().orderTax, "grand_total": grandTotal, "pay_method": paymentId, "paid": "COD", "save_date": Get.find<IndividualSlotController>().savedDate, "slot": Get.find<IndividualSlotController>().selectedSlotIndex, 'wallet_used': isWalletChecked == true && walletDiscount > 0 ? 1 : 0, 'wallet_price': isWalletChecked == true && walletDiscount > 0 ? walletDiscount : 0, "notes": notesEditor.text.isNotEmpty ? notesEditor.text : 'NA', "status": 0};
  ...
  Future<void> payWithInstaMojo() async {
    Get.dialog(
        ...
        barrierDismissible: false);
    var param = {'allow_repeated_payments': 'False', 'amount': grandTotal, 'buyer_name': parser.getName(), 'purpose': 'Orders', 'redirect_url': '${parser.apiService.appBaseUrl}/api/v1/success_payments', 'phone': parser.getPhone() != '' ? parser.getPhone() : '8888888888888888', 'send_email': 'True', 'webhook': parser.apiService.appBaseUrl, 'send_sms': 'True', 'email': parser.getEmail()};
