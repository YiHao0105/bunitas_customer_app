...
class PaymentController extends GetxController implements GetxService {
  ...
  void onCheckout() {
    if (paymentId == 1) {
      ...
      // stripe payment
      Get.toNamed(AppRouter.getStripePay(), arguments: [grandTotal, gateway!.currencyCode.toString(), 'salon']);
    } else if (paymentId == 3) {
  ...
  Future<void> createOrder() async {
    ...
    var param = {"uid": parser.getUID(), "freelancer_id": 0, "salon_id": Get.find<ServiceCartController>().salonId.toString(), "specialist_id": Get.find<SlotController>().selectedSpecialist, "appointments_to": appointmentsTo, "address": appointmentsTo == 1 ? jsonEncode(addressInfo) : 'NA', "items": jsonEncode(Get.find<ServiceCartController>().savedInCart), "coupon_id": selectedCoupon.code != null ? selectedCoupon.id : 0, "coupon": selectedCoupon.code != null ? jsonEncode(selectedCoupon) : 'NA', "discount": discount, "distance_cost": deliveryPrice, "total": Get.find<ServiceCartController>().totalPrice, "serviceTax": Get.find<ServiceCartController>().orderTax, "grand_total": grandTotal, "pay_method": paymentId, "paid": "COD", "save_date": Get.find<SlotController>().savedDate, "slot": Get.find<SlotController>().selectedSlotIndex, 'wallet_used': isWalletChecked == true && walletDiscount > 0 ? 1 : 0, 'wallet_price': isWalletChecked == true && walletDiscount > 0 ? walletDiscount : 0, "notes": notesEditor.text.isNotEmpty ? notesEditor.text : 'NA', "status": 0};
